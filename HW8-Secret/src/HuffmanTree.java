...
//
//
// Note, slight modifications had to made for compatibility with HuffMain2.  
// Mainly, the root field was changed from private to protected, and the meat of the 
// constructor was migrated over to a helper method so it can be accessed at any 
// time by the object.
...
public class HuffmanTree {
	protected HuffmanNode root;	// Root of our Huffman Tree.  Lonely field is lonely :(
	...
	public HuffmanTree(Map<Character, Integer> counts) {
		...
		// Create a new tree and save it to root.
		changeRoot(counts);
	}
	...
	// Slight structure change so we can modify the root node at any time.
	// This is useful for the decompression case in HuffmanTree2 where the initial root
	// is simply the EOF character.
	//
	// Precondition:  Passed map isn't null.
	// Type is protected so it can be accessed by subclasses like HuffmanTree2
	protected void changeRoot(Map<Character, Integer> counts) {
