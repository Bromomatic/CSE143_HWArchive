...
//
// A QuestionTree object allows a client program to maintain a tree used for playing
// games of 20 questions (even though there's no limit to the depth of the tree or how
// many questions we can ask the user... gg Marty).  A program can use this object to play
// a game (which can self-build a tree based on input from the user), load a pre-made tree
// that's in the form of a formatted text file, save the current tree into the same format, 
// get the number of total games, and/or get the number of games won in a sitting of the game.
...
public class QuestionTree {
	...
	// Plays a single, complete guessing game with the user.  Game begins with the root
	// node and moves left/right based on user answers.  If the tree guesses incorrectly,
	// the user is prompted for the correct answer, a question that would lead to that answer,
	// and whether the answer to the question is yes or no.  This is then added to the tree.
	//
	...
	// Does the actual work for the play method; accepts a QuestionNode and returns a
	...
	private QuestionNode play(QuestionNode curr) {
		...
		if (curr.isAnswer()) {
			...
			// Note, we need to add a question node AND assign a node to it left/right field,
	...
	private QuestionNode fixNode(String object, String question, boolean direction, 
			                                                             QuestionNode curr) {
	...
	// Replaces the current tree with a tree generated by reading the passed scanner.
	...
	// Precondition:  the file exists and is in the proper format.  Scanner is not null.
	...
	// Accepts a printstream and a node; prints its data and children in preorder order.
	// Precondition:  Passed printstream is not null.
	...
	// Simply returns how many correct guesses have been made by the tree in this instance.
	public int gamesWon() {
		return gamesWon;
	}
}